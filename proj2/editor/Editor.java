...
public class Editor extends Application {
    ...
    private class KeyEventHandler implements EventHandler<KeyEvent> {
        ...
        public void setFinalTexts(double x, double y , String text) {
         //    int size = texts.size();
        	// if(size != 0 && size%80 == 0) {
        	// 	texts.add("\n");
        	// }
            Text newText = new Text(x, y, text.toLowerCase());
            texts.add(newText);
        	String result = String.join("", texts.typedText);
        	displayText.setText(result);
        }

        public int calculateX() {
            return STARTING_FONT_SIZE*(texts.size()-1);
        }

        public int calculateY() {
            int size = texts.size();
            int lines = size/80;
            return lines*STARTING_FONT_SIZE;
        }
        ...
        public void handle(KeyEvent keyEvent) {
            if (keyEvent.getEventType() == KeyEvent.KEY_TYPED) {
                ...
                if (characterTyped.length() > 0) {
                    ...
                    setFinalTexts(calculateX(texts),calculateY(texts),characterTyped);
                    keyEvent.consume();
                }
                centerText();
